...
from ...core.visualizer import show_3d_bev
...
class EProPnPDet(SingleStageDetector):
    ...
    def show_result(self,
        ...
        if 'x3d' in views:
            ...
            x3d_bev = show_3d_bev(
                ori_img, result['x3d'], result['bbox_3d_results'], cam_intrinsic, 
                width=ori_img.shape[1], height=img.shape[0], scale=bev_scale,
                score_thr=score_thr, thickness=2)
            img_show.append(x3d_bev)
        if show:
            if len(img_show) == 1:
                img_show = img_show[0]
            elif len(img_show) == 2:
        ...
        if out_file is not None:
            ...
            if 'x3d' in views:
                mmcv.imwrite(x3d_bev, out_file[:-4] + '_x3d_bev.png')
            if '2d' in views:
