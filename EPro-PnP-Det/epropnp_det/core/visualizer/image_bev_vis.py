...
def show_3d_bev(
    img, x3d_result, bbox_3d_results, cali_mat, width=None, height=None,
    scale=10, score_thr=0.1, thickness=2,
    frustum_color=(220, 220, 220), box_color=(10, 60, 240), gt_box_color=(20, 180, 20),
    sample_color=(0.9, 0.5, 0.1), intensity_scale=40, sample_size=3, range_mark=[10, 20, 30, 40, 50, 60]):
    if (width is None) or (height is None):
        height, width = img.shape[:2]
    ...
    if x3d_result is not None and bbox_3d_results is not None:
        bbox_3d_results = np.concatenate(bbox_3d_results, axis=0)
        x3d_result = np.concatenate(x3d_result, axis=0)
        score = bbox_3d_results[:, 7]
        mask = score >= score_thr
        x3d_result = x3d_result[mask]
        x3d_bev = (np.round(x3d_result[..., [0, 2]] @ proj_mat.T) + origin).astype(np.int64)
        in_bev_mask = (x3d_bev >= 0).all(axis=-1) & (x3d_bev[..., 0] < width) & (x3d_bev[..., 1] < height)
        x3d_bev = x3d_bev[in_bev_mask]
        sample_inds = x3d_bev[:, 1] * width + x3d_bev[:, 0]
        density = np.bincount(
            sample_inds, minlength=height * width
        ).reshape(height, width)
        density = cv2.blur(density, [sample_size, sample_size]) * (sample_size * sample_size)
        colored_density = np.power(sample_color, density[..., None])
        bev_img = (bev_img * colored_density).astype(np.uint8)
